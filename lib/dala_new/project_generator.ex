...
defmodule DalaNew.ProjectGenerator do
  ...
  defp patch_mix_exs(project_dir, opts) do
    ...
    if File.exists?(path) do
      ...
      patched = DalaNew.LiveViewPatcher.inject_deps(content, a.dala_dep, a.dala_dev_dep)
  ...
  defp apply_liveview_patches(app_name, project_dir, opts) do
    ...
    # 1. Inject DalaHook into assets/js/app.js
    ...
    generate_dala_screen(project_dir, app_name, module_name)

    # 4. Generate lib/<app>/dala_app.ex (BEAM entry point for LiveView mode)
    generate_dala_live_app(project_dir, app_name, module_name)
    ...
    write_dala_exs(project_dir, a.dala_exs_dala_dir, a.dala_exs_elixir_lib)

    # 7b. Patch Phoenix's config files to use 4200 (dev) and 4202 (test) so
    #     `mix phx.server` won't collide with another LV project on 4000.
    #     The on-device runtime in dala_app.ex already uses 4200 — this lines
  ...
  defp patch_app_js(project_dir) do
    ...
    if File.exists?(path) do
      content = File.read!(path)
      patched = DalaNew.LiveViewPatcher.inject_dala_hook(content)
      File.write!(path, patched)
      ...
      Mix.shell().info([
        ...
        "* skip ",
        :reset,
        "assets/js/app.js not found — add DalaHook manually"
      ])
    end
  end
  ...
  defp patch_root_html(project_dir, app_name) do
    ...
    if path do
      content = File.read!(path)
      patched = DalaNew.LiveViewPatcher.inject_dala_bridge_element(content)
      File.write!(path, patched)
      Mix.shell().info([:green, "* patch ", :reset, path, " (added dala-bridge element)"])
    else
      Mix.shell().info([
        :yellow,
        "* skip root.html.heex not found — add dala-bridge element manually:",
        :reset
      ])

      Mix.shell().info("    " <> DalaNew.LiveViewPatcher.dala_bridge_element())
    end
  end

  defp generate_dala_screen(project_dir, app_name, module_name) do
    dir = Path.join([project_dir, "lib", app_name])
    path = Path.join(dir, "dala_screen.ex")
    File.mkdir_p!(dir)
    File.write!(path, DalaNew.LiveViewPatcher.dala_screen_content(module_name))
  ...
  defp generate_dala_live_app(project_dir, app_name, module_name) do
    secret_key_base = generate_secret_key_base()
    signing_salt = generate_signing_salt()

    content =
      DalaNew.LiveViewPatcher.dala_live_app_content(
        module_name,
        app_name,
        secret_key_base,
        signing_salt
  ...
  # standardises on 4200 (host dev + on-device runtime — see dala_app.ex), so
  # `mix phx.server` from a generated project won't collide with someone
  ...
  defp generate_notes_app(project_dir, app_name, module_name) do
    ...
    write.(
      ...
      DalaNew.LiveViewPatcher.repo_content(module_name, app_name)
    )

    write.(
      Path.join(lib_dir, "note.ex"),
      DalaNew.LiveViewPatcher.note_content(module_name)
    )

    write.(
      Path.join(lib_dir, "notes.ex"),
      DalaNew.LiveViewPatcher.notes_content(module_name, app_name)
    )

    write.(
      Path.join(migrations_dir, "20260424000000_create_notes.exs"),
      DalaNew.LiveViewPatcher.migration_content(app_name)
    )

    write.(
      Path.join(live_dir, "notes_list_live.ex"),
      DalaNew.LiveViewPatcher.notes_list_live_content(module_name, app_name)
    )

    write.(
      Path.join(live_dir, "note_editor_live.ex"),
      DalaNew.LiveViewPatcher.note_editor_live_content(module_name, app_name)
    )

    write.(
      Path.join(live_dir, "about_live.ex"),
      DalaNew.LiveViewPatcher.about_live_content(module_name, app_name)
    )
  ...
  defp overwrite_liveview_build_sh(project_dir, app_name, module_name) do
    ...
    File.write!(path, DalaNew.LiveViewPatcher.liveview_build_sh_content(module_name, app_name))
  ...
  defp generate_erlang_entry(project_dir, app_name, module_name) do
    ...
    File.mkdir_p!(dir)
    File.write!(path, DalaNew.LiveViewPatcher.erlang_entry_content(module_name, app_name))
    Mix.shell().info([:green, "* create ", :reset, path])
  end
  ...
  defp resolve_deps(opts) do
    if opts[:local] do
      ...
      dala_dep = ~s({:dala,     "~> 0.1.0"})
      dala_dev_dep = ~s({:dala_dev, "~> 0.1.0", only: :dev, runtime: false})
      dala_exs_dala_dir = "Path.join(File.cwd!(), \"deps/dala\")"

      dala_exs_elixir_lib =
        "System.get_env(\"DALA_ELIXIR_LIB\", System.get_env(\"HOME\") <> \"/.local/share/mise/installs/elixir/1.18.4-otp-28/lib\")"
